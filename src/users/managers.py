...
class User_QuerySet(models.QuerySet):
    ...
    def motorizations___car_user__in(self):
        """
        Get records into motorizations,Car_user
        """
    ...
    def motorizations___car_user__exclude(self):
        """
        Get records not into motorizations,Car_user
        """
        from .models import User
        
        return self.exclude(id__in=Subquery(
                User.objects.motorizations___car_user__in().values('id')
            )
        )
        
    
    def motorizations___car_user__number_plate__contains(self, number_plate):
        """
        Get records into motorizations, Car_user whitch contains number_plate
        """
        from motorizations.models import Car_user
